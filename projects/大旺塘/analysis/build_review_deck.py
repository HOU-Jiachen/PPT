...
from engineering_deck_runtime import (  # noqa: E402
    ...
    load_json,
    report_heading,
    report_points_from_evidence,
    report_text_from_evidence,
    rgb,
...
def add_cover_slide(slide) -> None:
    ...
    add_textbox(slide, 0.78, 0.72, 4.7, 0.28, "华润水泥（封开）有限公司", 12, "paper", colors=COLORS)
    ...
    add_textbox(slide, 0.84, 4.42, 10.8, 0.36, "依据报告章节、原始图表、计算口径和防治水建议形成评审级汇报", 13, "paper", colors=COLORS)
    add_textbox(slide, 0.86, 6.72, 4.2, 0.3, datetime.now().strftime("%Y年%m月"), 11, "paper", colors=COLORS)
...
def add_calculation_slide(
    ...
    add_textbox(slide, 0.85, 1.45, 4.9, 0.3, report_heading(page["source_mode"]), 16, "accent", True, colors=COLORS)
    add_textbox(
        slide,
        0.85,
        1.9,
        4.9,
        1.25,
        text_preview("；".join(formulas) or body_text or "报告按公式、参数表和结果表列示计算过程。", 280),
        12.8,
        "ink",
        colors=COLORS,
    )
    ...
    if table_entry:
        ...
        add_textbox(slide, 6.45, 1.3, 5.5, 3.5, text_preview(body_text, 620), 13.5, "ink", colors=COLORS)
...
def add_source_slide(
    ...
    if page["source_mode"] == "ORIGINAL_FIGURE" or image_paths:
        ...
        add_textbox(slide, 8.35, 1.18, 4.25, 0.32, report_heading(page["source_mode"]), 15, "accent", True, colors=COLORS)
        add_textbox(slide, 8.35, 1.58, 4.15, 3.75, text_preview(body_text or page["title"], 620), 13.5, "ink", colors=COLORS)
        ...
        add_textbox(slide, 9.25, 1.25, 3.2, 0.32, report_heading(page["source_mode"]), 15, "accent", True, colors=COLORS)
        add_textbox(slide, 9.25, 1.65, 3.1, 3.45, text_preview(body_text or table_entry.get("text", ""), 520), 12.8, "ink", colors=COLORS)
    elif page["source_mode"] == "CALCULATION":
        add_calculation_slide(slide, page, evidence, body_text, table_entry)
    else:
        add_fill(slide, 0.65, 1.25, 12.0, 4.9, "paper", colors=COLORS)
        add_textbox(slide, 0.95, 1.55, 11.35, 0.35, report_heading(page["source_mode"]), 16, "accent", True, colors=COLORS)
        points = report_points_from_evidence(evidence, catalog_entries, max_points=5)
        if len(points) >= 2:
            add_bullets(slide, 0.95, 2.08, 10.95, 3.30, points, 14.0, colors=COLORS)
        else:
            add_textbox(slide, 0.95, 2.08, 11.1, 3.15, text_preview(body_text or page["title"], 720), 15, "ink", colors=COLORS)

    add_footer(slide, page.get("source_note", ""), colors=COLORS)
...
def build_deck() -> Path:
    ...
    for page in plan["slides"]:
        ...
        if page["page"] == 1:
            ...
            add_source_slide(slide, page, evidence, catalog_entries, figure_index)
